  ...
  head: () => ({
    meta: [
      { title: "Relatórios da IA — Disciplina Absoluta" },
      {
        name: "description",
        content:
          "Análises de produtividade, padrões de horário, abandono por categoria e evolução.",
      },
      { property: "og:title", content: "Relatórios da IA" },
      {
        property: "og:description",
        content: "Descubra seus padrões de produtividade e onde você perde tempo.",
      },
  ...
  return (
    <div className="px-5 pt-6 pb-24 animate-rise">
      ...
      <p className="text-xs text-muted-foreground mb-6">
        O que seus dados dizem sobre a sua disciplina.
      </p>

      <div className="grid grid-cols-2 gap-3 mb-6">
        <Card
          label="Tempo total focado"
          value={`${Math.floor(report.totalMinutes / 60)}h ${report.totalMinutes % 60}m`}
        />
        <Card label="Tempo economizado" value={`${report.savedMinutes} min`} accent />
        <Card
          label="Melhor horário"
          value={report.bestHour ? `${String(report.bestHour.hour).padStart(2, "0")}:00` : "—"}
        />
      ...
      {report.topTask && (
        <p className="text-xs text-muted-foreground mb-6">
          Tarefa mais concluída: <b className="text-foreground">{report.topTask.name}</b> (
          {report.topTask.count}x)
        </p>
      )}
      ...
      {report.worstCategory && (
        <div className="flex gap-3 bg-struggle/10 border border-struggle/25 rounded-2xl p-4 mb-6">
          ...
          <p className="text-xs text-struggle/90 leading-snug">
            Maior taxa de abandono: <b className="uppercase">{report.worstCategory.category}</b> —{" "}
            {report.worstCategory.rate}% das ocorrências planejadas nos últimos 30 dias não foram
            concluídas.
          </p>
        </div>
      )}
      ...
      <Section icon={<Clock className="size-4" />} title="Tempo por categoria">
        <div className="space-y-2">
          {report.minutesByCategory.length === 0 && (
            ...
            <div key={cat} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-2 bg-surface rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.max((min / (report.minutesByCategory[0]?.[1] || 1)) * 100, 3)}%`,
                  }}
                />
              </div>
        ...
        <div className="space-y-3">
          {suggestions.length === 0 && (
            <p className="text-xs text-muted-foreground">
              Nenhum ajuste sugerido. Sua agenda está coerente.
            </p>
          )}
          {suggestions.map((s) => (
...
function Card({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-4">
      <div className={`text-xl font-heading font-black mb-1 ${accent ? "text-discipline" : ""}`}>
        {value}
      </div>

...
function MonthView() {
  ...
  const cells = useMemo(() => {
    ...
    while (arr.length % 7 !== 0 || arr.length < 42) {
      ...
      arr.push({
        date: dateKey(nd),
        day: nd.getDate(),
        inMonth: nd.getMonth() === cursor.getMonth(),
      });
  ...
  const shiftYear = (n: number) =>
    setCursor(new Date(cursor.getFullYear() + n, cursor.getMonth(), 1));
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="text-center min-w-[140px]">
            <div className="text-lg font-heading font-black uppercase leading-none">
        ...
        {cells.map((c) => {
          ...
          const colorClass =
            stats.total === 0
              ? "border-border"
              : stats.pct === 100
                ? "border-discipline/60 bg-discipline/10"
                : stats.pct > 0
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                <span className={`text-xs font-bold ${isToday ? "text-discipline" : ""}`}>
                  {c.day}
                </span>
                {stats.total > 0 && (
      ...
      <div className="mt-4 flex items-center justify-around text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
        <span className="flex items-center gap-1">
          <span className="size-2 rounded-full bg-discipline" /> 100%
        </span>
        <span className="flex items-center gap-1">
          <span className="size-2 rounded-full bg-warning" /> Parcial
        </span>
        <span className="flex items-center gap-1">
          <span className="size-2 rounded-full bg-struggle" /> Pendente
        </span>
      </div>
    </div>
  );

...
const statuses: (TaskStatus | "todas")[] = [
  "todas",
  "nao-iniciada",
  "em-andamento",
  "concluida",
...
function SearchView() {
  ...
  const results = useMemo(() => {
    ...
    return tasks.filter((t) => {
      if (
        term &&
        !`${t.name} ${t.description ?? ""} ${t.notes ?? ""} ${t.reward} ${t.consequence}`
          .toLowerCase()
          .includes(term)
      )
        return false;
      ...
      <div className="space-y-2">
        {results.map((t) => (
          <button
            key={t.id}
            onClick={() =>
              navigate({ to: "/calendar/day/$date", params: { date: t.scheduledDate } })
            }

...
export function TaskForm({
  ...
  const submit = () => {
    ...
    // Sanitiza os campos numéricos: evita NaN/0 quando o input é apagado.
    const safeEstimated = Number.isFinite(estimated) && estimated > 0 ? Math.round(estimated) : 30;
    const safeMax =
      Number.isFinite(max) && max > 0 ? Math.max(Math.round(max), safeEstimated) : safeEstimated;
    if (!Number.isFinite(estimated) || estimated <= 0) {
      toast.error("Informe um tempo estimado válido.");
      return;
    }
    onSubmit({
      name: name.trim(),
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Estimado (min)">
          <input
            type="number"
            min={1}
            value={Number.isFinite(estimated) ? estimated : ""}
            onChange={(e) => setEstimated(e.target.value === "" ? NaN : Number(e.target.value))}
            className="w-full bg-surface border border-border rounded-xl px-3 py-3 focus:outline-none focus:border-discipline"
          />
        </Field>
      </div>
      ...
      <Field label="Máximo (min)">
        <input
          ...
          onChange={(e) => setMax(e.target.value === "" ? NaN : Number(e.target.value))}
          className="w-full bg-surface border border-border rounded-xl px-3 py-3 focus:outline-none focus:border-discipline"
        />
      </Field>
          ...
          <div className="mt-3 space-y-2">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-discipline text-black rounded-lg text-xs font-bold"
              >
                Add
              </button>
            </div>
      ...
      <Field label="Data personalizada — período ativo (opcional)">
        ...
        <p className="text-[10px] text-muted-foreground mt-1">
          A tarefa só aparece nesse intervalo. Após o fim, some das Missões de Hoje (fica no
          histórico).
        </p>
      </Field>
      ...
      <Field label="Cor">
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`size-8 rounded-full border-2 ${!color ? "border-discipline" : "border-border"} bg-transparent text-[10px]`}
          >
            —
          </button>
          {colors.map((c) => (
      ...
      <Field label="Ícone">
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`size-9 rounded-lg border ${!icon ? "border-discipline" : "border-border"} bg-surface text-xs`}
          >
            —
          </button>
          {icons.map((i) => (
            <button
              key={i}
              onClick={() => setIcon(i)}
              className={`size-9 rounded-lg border text-lg ${icon === i ? "border-discipline bg-discipline/10" : "border-border bg-surface"}`}
            >
              {i}
            </button>
          ))}
      ...
      <Field label="Frase motivacional (opcional)">
        <input
          ...
          placeholder="Ex.: Disciplina supera motivação."
          className="w-full bg-surface border border-border rounded-xl px-4 py-3 focus:outline-none focus:border-discipline"
        />
      </Field>
      ...
      <Field label="Recompensa (se cumprir)">
        <input
          ...
          placeholder="Ex.: 1 episódio da série"
          className="w-full bg-surface border border-border rounded-xl px-4 py-3 focus:outline-none focus:border-discipline"
        />
      </Field>
      ...
      <Field label="Consequência (se falhar)">
        <input
          ...
          placeholder="Ex.: Sem redes sociais hoje"
          className="w-full bg-surface border border-border rounded-xl px-4 py-3 focus:outline-none focus:border-discipline"
        />
      </Field>
